import crypto from 'crypto';

export default async function handler(req, res) {
    if (req.method !== 'POST') return res.status(405).json({ error: 'Método não permitido' });

    // A chave pública gigante
    const publicKey = "";
    
    // A sua API Key EXCLUSIVA que você gerou agora
    const apiKey = "9TdohQd9ortOeqcJquB5XaFxRWdjaaOX";

    try {
        const pemKey = `-----BEGIN PUBLIC KEY-----\n${publicKey.match(/.{1,64}/g).join('\n')}\n-----END PUBLIC KEY-----`;
        
        const encryptedKey = crypto.publicEncrypt(
            { key: pemKey, padding: crypto.constants.RSA_PKCS1_PADDING }, 
            Buffer.from(apiKey)
        ).toString('base64');

        // URL corrigida para vodacomMOZ
        const response = await fetch('https://openapi.m-pesa.com/sandbox/ipg/v2/vodacomMOZ/getSession/', {
            method: 'GET',
            headers: { 'Authorization': `Bearer ${encryptedKey}`, 'Origin': '*' }
        });

        const data = await response.json();
        res.status(200).json(data);
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
}
